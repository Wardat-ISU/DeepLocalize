...
def make_model(model_name,model_desc,num_features):
    # Building model: 
    #print("desc = parse_desc(model_desc)")
    adam = keras.optimizers.Adam() # Does converge slowly 
    melbourne_model = keras.models.Sequential(name="-"+model_name + "-Adam")
    melbourne_model.add(keras.layers.Dense(20,kernel_initializer='glorot_normal',
	input_dim=num_features))
    melbourne_model.add(keras.layers.Activation('tanh'))
    melbourne_model.add(keras.layers.Dense(20, kernel_initializer='glorot_normal'))
    melbourne_model.add(keras.layers.Activation('tanh'))
    melbourne_model.add(keras.layers.Dense(1 ))   # MAE: 922165
    melbourne_model.add(keras.layers.Activation('linear'))
    

    melbourne_model.compile(loss=args.loss, optimizer=adam, metrics= ['accuracy']) # Does converge slowly
    melbourne_model.summary()
...
def train(model_name, model_desc, num_epochs, X, y):
    ...
    if args.checkpoints == True:
        ...
        history = melbourne_model.fit(X.values, y.values, validation_split=0.2, epochs=num_epochs, initial_epoch=initial_epoch, batch_size=1,
                                      #callbacks=[show_stopper,checkpoint]
                                      callbacks= [
                                        #keras.callbacks.TerminateOnNaN(),
                                        #keras.callbacks.EarlyStopping(monitor='loss', patience=1),
                                        #keras.callbacks.EarlyStopping(monitor='accuracy', patience=1)
                                          keras.callbacks.backpropagation(X.values, y.values, len(melbourne_model.layers), 1,start_time)
                                        ])
    else:
        history = melbourne_model.fit(X.values, y.values, validation_split=0.2, epochs=num_epochs, initial_epoch=initial_epoch, batch_size=1,
                                      #callbacks=[show_stopper]
                                      callbacks= [
                                        #keras.callbacks.TerminateOnNaN(),
                                        #keras.callbacks.EarlyStopping(monitor='loss', patience=1),
                                        #keras.callbacks.EarlyStopping(monitor='accuracy', patience=1)
